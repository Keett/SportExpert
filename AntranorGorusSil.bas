...
Sub Globals
	...
	Private ButtonAntranorGorusEkle As Button
	Private ButtonAntranorGorusTabloVeri As Button
...
Sub Activity_Create(FirstTime As Boolean)
	...
	Activity.LoadLayout("AntranorGorusleriSilPage")
...
Public Sub VeriAl
	id = rs.GetInt("antranorGorüsleri_id")
	tc = rs.GetString("denek_tc")
	isim = rs.GetString("denek_isim")
	tarih = rs.GetString("antranorGorüsleri_tarih")
	saat = rs.GetString("antranorGorüsleri_saat")
	aciklama = rs.GetString("antranorGorüsleri_aciklama")
...
Sub ListViewGosterDB
	WebView1.Visible = False
	ListView1.Clear
	rs = Main.sql.ExecQuery("SELECT * FROM AntranorGorusleri,Denek WHERE AntranorGorusleri.denek_id = Denek.denek_id ORDER BY antranorGorüsleri_id DESC")
...
Sub EditTextAra_TextChanged (Old As String, New As String)
	...
	ListView1.Clear
	rs = Main.sql.ExecQuery( "SELECT * FROM AntranorGorusleri,Denek WHERE AntranorGorusleri.denek_id = Denek.denek_id AND (denek_tc LIKE '%"& EditTextAra.Text &"%' OR denek_isim LIKE '%"& EditTextAra.Text &"%' OR antranorGorüsleri_tarih LIKE '%"& EditTextAra.Text &"%' OR antranorGorüsleri_aciklama LIKE '%"& EditTextAra.Text &"%') ORDER BY antranorGorüsleri_id DESC")
...
Sub ButtonTarih_Click
	WebView1.Visible = False
	ListView1.Clear
	rs = Main.sql.ExecQuery("SELECT * FROM AntranorGorusleri,Denek WHERE AntranorGorusleri.denek_id = Denek.denek_id AND antranorGorüsleri_tarih  LIKE '%"& EditTextAra.Text&"%' ORDER BY antranorGorüsleri_id DESC ")
...
Sub ButtonAntranorGorusAciklama_Click
	WebView1.Visible = False
	ListView1.Clear
	rs = Main.sql.ExecQuery("SELECT * FROM AntranorGorusleri,Denek WHERE AntranorGorusleri.denek_id = Denek.denek_id AND antranorGorüsleri_aciklama LIKE '%"& EditTextAra.Text&"%' ORDER BY antranorGorüsleri_id DESC")
...
Sub ButtonTc_Click
	WebView1.Visible = False
	ListView1.Clear
	rs = Main.sql.ExecQuery("SELECT * FROM AntranorGorusleri,Denek WHERE AntranorGorusleri.denek_id = Denek.denek_id AND denek_tc LIKE '%"& EditTextAra.Text &"%' ORDER BY antranorGorüsleri_id DESC")
...
Sub ButtonIsim_Click
	WebView1.Visible = False
	ListView1.Clear
	rs = Main.sql.ExecQuery("SELECT * FROM AntranorGorusleri,Denek WHERE AntranorGorusleri.denek_id = Denek.denek_id AND denek_isim LIKE '%"& EditTextAra.Text &"%' ORDER BY antranorGorüsleri_id DESC")
...
Sub ButtonAntranorGorusEkle_Click
	WebView1.Visible = False
	StartActivity(AntropometrikEkle)
	Activity.Finish
End Sub

Sub ButtonAntranorGorusTabloVeri_Click
	WebView1.Visible = False
...
Sub ListView1_ItemClick (Position As Int, Value As Object)
	...
	WebView1.LoadHtml(DBUtils.ExecuteHtml(Main.sql,"SELECT denek_tc As [Denek TC], denek_isim As [Denek İSİM],antranorGorüsleri_tarih As [TARİH], antranorGorüsleri_saat As [SAAT], antranorGorüsleri_aciklama As [ANTRANOR GÖRÜŞLERİ AÇIKLAMA] FROM AntranorGorusleri,Denek WHERE AntranorGorusleri.denek_id = Denek.denek_id AND antranorGorüsleri_id = '"&Value&"' ",Null,-1,False))
	Return
End Sub

Sub ListView1_ItemLongClick (Position As Int, Value As Object)
	rs = Main.sql.ExecQuery("SELECT * FROM AntranorGorusleri WHERE antranorGorüsleri_id  = '"&Value&"' ")
	...
	Dim result As Int
	result = Msgbox2("Antranor görüşleri silmek istediğinizden emin misiniz?:", "Onay Mesajı", "Evet", "", "Hayır",Null)
	If result = DialogResponse.POSITIVE Then
		Main.sql.ExecNonQuery("DELETE FROM AntranorGorusleri WHERE antranorGorüsleri_id = '"&id&"' ")
		ToastMessageShow("Antranor Gorusleri silindi...",False)
	End If
	ListViewGosterDB
End Sub
